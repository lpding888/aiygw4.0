 ...
 */
function parseArgs() {
  const args = { yes: false, help: false };
 ...
 * 查询快照列表
 */
async function listSnapshots(scope, key) {
 ...
 * 提示用户选择快照
 */
async function selectSnapshot(snapshots) {
 ...
 * 二次确认
 */
async function confirmRollback(snapshot) {
  return new Promise((resolve) => {
    const rl = readline.createInterface({
      input: process.stdin,
      output: process.stdout
 ...
 * 执行回滚操作
 */
async function performRollback(snapshot) {
