...
class SystemConfigController {
  async getValue(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const { defaultValue = null } = req.query as Record<string, any>;
      const value = await (systemConfigService as any).get(key, defaultValue);
  ...
  async getCategory(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const { includeSecrets = false } = req.query as Record<string, any>;
      const configs = await (systemConfigService as any).getByCategory(
        category,
        includeSecrets === 'true'
      );
  ...
  async list(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await (systemConfigService as any).list({
        category,
  ...
  async getCategories(_req: Request, res: Response, next: NextFunction) {
    try {
      const categories = await (systemConfigService as any).getCategories();
  ...
  async setValue(req: Request, res: Response, next: NextFunction) {
    try {
      const { key } = req.params as { key: string };
      const { value, type = 'string', description = '' } = req.body as any;
      const userId = req.user?.id as string | undefined;
      if (value === undefined) {
        res.status(400).json({ success: false, error: { code: 4001, message: '配置值不能为空' } });
        return;
      }
      await (systemConfigService as any).set(key, value, type, description, userId);
      ...
      if (String(error?.message || '').includes('系统配置不可删除')) {
        res.status(403).json({ success: false, error: { code: 4003, message: error.message } });
        return;
      }
      next(error);
    }
  ...
  async setBatch(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      await (systemConfigService as any).setMultiple(configs, userId);
      res.json({
        success: true,
  ...
  async delete(req: Request, res: Response, next: NextFunction) {
    try {
      const { key } = req.params as { key: string };
      const userId = req.user?.id as string | undefined;
      const deleted = await (systemConfigService as any).delete(key, userId);
      if (!deleted) {
  ...
  async reloadCache(_req: Request, res: Response, next: NextFunction) {
    try {
      await (systemConfigService as any).reloadCache();
      res.json({ success: true, message: '配置缓存已重载' });
    } catch (error: any) {
      logger.error(`[SystemConfigController] 重载缓存失败: ${error?.message}`, error);
      next(error);
    }
  }
  ...
  async export(_req: Request, res: Response, next: NextFunction) {
    try {
      const configs = await (systemConfigService as any).export();
      const exportData = (configs || []).map((c: any) => ({
        key: c.key,
        value: c.value,
        type: c.type,
  ...
  async import(req: Request, res: Response, next: NextFunction) {
    try {
      const { configs, overwrite = false } = req.body as any;
      const userId = req.user?.id as string | undefined;
      if (!Array.isArray(configs) || configs.length === 0) {
        res
          .status(400)
      ...
      for (const config of configs) {
        try {
          ...
          const existing = await (systemConfigService as any).get(key);
          ...
          results.failed++;
          results.errors.push(`配置${config.key}导入失败: ${err.message}`);
        }
      }
