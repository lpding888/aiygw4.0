...
class InviteCodeController {
  async generateInviteCodes(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      logger.info(
        `[InviteCodeController] User ${(req as any).user?.id || 'system'} generated ${codes.length} invite codes`
      );
      ...
      res.json({
        ...
        data: {
          ...
          summary: {
            ...
            expiresAt: (codes?.[0] as any)?.expires_at
          }
        }
      });
    } catch (error) {
      logger.error('[InviteCodeController] Failed to generate invite codes:', error as any);
      next(error);
    }
  }
  ...
  async validateInviteCode(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      logger.error('[InviteCodeController] Failed to validate invite code:', error as any);
      next(error);
    }
  }
  ...
  async useInviteCode(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      logger.error('[InviteCodeController] Failed to use invite code:', error as any);
      next(error);
    }
  }
  ...
  async getInviteCodes(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      logger.error('[InviteCodeController] Failed to get invite codes:', error as any);
      next(error);
    }
  }
  ...
  async getUserInviteStats(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const targetUserId = (req.params?.userId as string | undefined) ?? (req as any).user?.id;
      const stats = await inviteCodeService.getUserInviteStats(targetUserId);
      res.json({ success: true, data: stats });
    } catch (error) {
      logger.error('[InviteCodeController] Failed to get user invite stats:', error as any);
      next(error);
    }
  }
  ...
  async getInviteUsageLogs(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      logger.error('[InviteCodeController] Failed to get invite usage logs:', error as any);
      next(error);
    }
  }
  ...
  async disableInviteCode(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      logger.error('[InviteCodeController] Failed to disable invite code:', error as any);
      next(error);
    }
  }
  ...
  async getInviteCodeStats(_req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      logger.error('[InviteCodeController] Failed to get invite code stats:', error as any);
      next(error);
    }
  }
  ...
  async cleanupExpiredCodes(_req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      logger.error('[InviteCodeController] Failed to cleanup expired codes:', error as any);
      next(error);
    }
  }
}
