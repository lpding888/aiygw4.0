...
class AIModelService {
  constructor() {
    this.config = {
      ...
      webappId: process.env.RUNNING_HUB_WEBAPP_ID || '1982694711750213634',
      apiKey: process.env.RUNNING_HUB_API_KEY || '0e6c8dc1ed9543a498189cbd331ae85c',
      timeout: 180000 // 3分钟
    };

    // 12分镜Prompt模板
    // 注意: 这些模板会传递给RunningHub的节点103(text字段)
   ...
   * @returns {string} Prompt文本(传递给RunningHub节点103)
   */
  generatePrompt(scene, category) {
    const prompt = this.promptTemplates[scene]?.[category];
    if (!prompt) {
  ...
  async createModelTask(taskId, inputImageUrl, params = {}) {
    try {
      ...
      // 生成Prompt文本
      const prompt = this.generatePrompt(scene, category);
  ...
  async submitToRunningHub(imageUrl, prompt) {
    try {
      ...
      logger.info('[AIModelService] 调用RunningHub API', { prompt, imageKey });
