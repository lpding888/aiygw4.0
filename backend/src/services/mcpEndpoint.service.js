...
class McpEndpointService {
  ...
  async getEndpoints(options = {}) {
    try {
      ...
      const db = require('../config/database');
      let query = db('mcp_endpoints')
        .select([
          'id', 'name', 'description', 'server_type', 'connection_string',
          'enabled', 'status', 'timeout', 'retry_count', 'last_connected_at',
          'last_tested_at', 'server_info', 'available_tools', 'created_at', 'updated_at'
        ]);
  ...
  getEndpointsFromMemory(options) {
    ...
    if (options.search) {
      endpoints = endpoints.filter(e =>
        e.name.includes(options.search) || e.description.includes(options.search)
      );
    }

    // 排序
    endpoints.sort((a, b) => new Date(b[options.sortBy || 'created_at']) - new Date(a[options.sortBy || 'created_at']));
  ...
  async updateTestResult(id, result) {
    try {
      ...
      const db = require('../config/database');
      await db('mcp_endpoints').where('id', id).update({
  ...
  async saveDiscoveredTools(endpointId, tools) {
    try {
      if (!this.isDatabaseAvailable()) {
        logger.info(`[McpEndpointService] Database not available, skipping tool storage for ${endpointId}`);
        return;
      }
...
module.exports = new McpEndpointService();