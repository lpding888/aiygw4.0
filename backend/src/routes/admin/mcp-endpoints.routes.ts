...
import * as mcpEndpointsService from '../../services/mcp-endpoints.service.js';
...
import { validate, validateMcpEndpoint } from '../../middlewares/validate.middleware.js';
...
const createEndpointValidation = [
  ...
  body('protocolVersion')
    ...
    .withMessage('无效的协议版本'),
  body('capabilities').optional().isArray().withMessage('能力列表必须是数组'),
  body('timeoutMs')
    .optional()
...
const updateEndpointValidation = [
  param('id').notEmpty().withMessage('端点ID不能为空'),
  body('name').optional().isLength({ max: 100 }).withMessage('端点名称最多100个字符'),
  body('description').optional().isLength({ max: 500 }).withMessage('描述最多500个字符'),
  body('endpointUrl').optional().isURL().withMessage('端点URL格式无效'),
  body('protocolVersion')
    .optional()
    .isIn(['2024-11-05', '2024-10-07'])
    .withMessage('无效的协议版本'),
...
const testEndpointValidation = [param('id').notEmpty().withMessage('端点ID不能为空')];

const executeToolValidation = [
  param('id').notEmpty().withMessage('端点ID不能为空'),
  param('toolName').notEmpty().withMessage('工具名称不能为空'),
...
const queryValidation = [
  ...
  query('status').optional().isIn(['active', 'inactive', 'error']).withMessage('无效的状态值'),
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { page = '1', limit = '20', status, enabled, healthy } = req.query;
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const endpoint = await mcpEndpointsService.createEndpoint(
        ...
        req.user!.id // 艹，这个路由有auth中间件保护，user肯定存在！
      );
