...
exports.up = async function (knex) {
  // 1. 用户配置表 (P0)
  await createTableIfMissing(knex, 'user_configs', (table) => {
    table.string('id', 36).primary().defaultTo(knex.raw('(UUID())'));
    table.string('user_id', 32).notNullable().unique().comment('用户ID');
  ...
  // 2. 用户登录历史表 (P0)
  await createTableIfMissing(knex, 'user_login_history', (table) => {
    table.string('id', 36).primary().defaultTo(knex.raw('(UUID())'));
  ...
  // 3. 用户会员关系表 (P1)
  await createTableIfMissing(knex, 'user_memberships', (table) => {
    table.string('id', 36).primary().defaultTo(knex.raw('(UUID())'));
    table.string('user_id', 32).notNullable().comment('用户ID');
    ...
    table.string('order_id', 32).nullable().comment('订单ID');
  ...
  // 4. 用户内容表 (P1)
  await createTableIfMissing(knex, 'user_contents', (table) => {
    table.string('id', 36).primary().defaultTo(knex.raw('(UUID())'));
    table.string('user_id', 32).notNullable().comment('用户ID');
  ...
  // 5. 合作伙伴表 (P1)
  await createTableIfMissing(knex, 'partnerships', (table) => {
    table.string('id', 36).primary().defaultTo(knex.raw('(UUID())'));
    table.string('user_id', 32).notNullable().comment('用户ID');
