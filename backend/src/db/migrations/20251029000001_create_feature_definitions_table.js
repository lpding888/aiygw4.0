...
exports.up = function (knex) {
  return knex.schema
    .createTable('feature_definitions', function (table) {
      ...
      table
        .string('access_scope', 20)
        .defaultTo('plan')
        .comment('访问范围(plan=按套餐/whitelist=白名单)');
      table.text('allowed_accounts').nullable().comment('白名单用户ID数组(JSON字符串)');
      table.integer('quota_cost').notNullable().comment('配额消耗数量');
      table
        .string('rate_limit_policy', 100)
        .nullable()
        .comment('限流策略(hourly:30表示每小时30次)');
      table
        .string('output_type', 50)
        .notNullable()
        .comment('输出类型(singleImage/multiImage/video/textBundle/zip)');
